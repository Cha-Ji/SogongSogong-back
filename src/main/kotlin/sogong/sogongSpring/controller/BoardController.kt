...
class BoardController (
    ...
    val boardServiceEdit: BoardEditService,
    val boardPrintService : BoardPrintService) {
   ...
   @GetMapping(value = ["/post-auth", "/comment-auth"])
    fun editAuth(@RequestParam("user-id") userId: Long,
                 @RequestParam("post-id") postid: Long,
                 @RequestParam("comment-id") commentid: Long? = null
   ): Boolean = boardServiceEdit.editAuth(
       EditPostCommentAuthDto(
            postId = postid,
            userId = userId,
            commentId = commentid
       )
    ...
    fun deleteComment(@PathVariable commentId: Long, @RequestParam("post-id") postId: Long) {
        boardServiceEdit.deleteComment(DeleteCommentDto(commentId, postId))
    }
    ...
    fun printPost(@PageableDefault(size=20, sort=["postId"], direction= Sort.Direction.DESC) pageable: Pageable) =
        boardPrintService.printEntirePost(pageable)

    @GetMapping("/one-post/{postId}")
    fun printOnePost(@PathVariable postId:Long) = boardPrintService.printOnePost(postId)

    @GetMapping("/comment")
    fun printComment(@RequestParam("post-id") postId:Long) = boardPrintService.printComment(postId)
    ...
    ) = boardPrintService.printScrapLike(userId, scrapLike)

    @GetMapping("/hot-post")
    fun printHotPost(@PageableDefault(size=20, sort=["postId"], direction= Sort.Direction.DESC) pageable: Pageable) =
        boardPrintService.printHotPost(pageable).content //content만 적용

    @GetMapping("/best-post")
    fun printBestPost(@PageableDefault(size=20, sort=["postId"], direction= Sort.Direction.DESC) pageable: Pageable) =
        boardPrintService.printBestPost(pageable) //page 전체만 적용
}
